...
const TOOL_METADATA = {
  ...
  create_guided_lesson: {
    ...
    description: `Create or replace a complete guided lesson transactionally. Always declare lessonMode. Choose lesson-step and scene-beat counts from the teaching content: keep simple lessons concise, use 10, 15, or more beats when distinct concepts need them, and never add filler or compress concepts to meet a quota. Use explain when the learner asks to understand, learn, see, or be shown a concept: provide complete example code and progressive explanation beats, not a workspace full of TODOs. When an initial scene teaches source code, preserve Monaco precision by giving each visual explanation beat one semantic target for the exact token, single line, or contiguous multi-line range being discussed, together with the registered highlight, spotlight, focus, or pointer effects that communicate that target. Use practice only when the learner asks for an exercise, challenge, TODO, or hands-on work. Use mixed for a demonstration followed by a small validated exercise. For a final coding exercise, map the last beat to a lesson step with structural validation criteria and an editor-change interaction wait; those criteria gate Finish through automatic and manual validation. Use at most ${DEFAULT_SYSTEM_LIMITS.maxVisualGuideItems} final-exercise criteria and write each criterion's learner-visible requirement alongside its validator. Omit supportingItems from the final beat: Lessonique derives the numbered list from those criterion requirements in the same order, so the guide and validation progress cannot diverge. ${GUIDE_INLINE_CODE_SYNTAX_DESCRIPTION}`,
  },
  reset_classroom: {
