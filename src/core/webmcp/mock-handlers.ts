...
const TOOL_METADATA = {
  ...
  create_guided_lesson: {
    ...
    description: `Create or replace a complete guided lesson transactionally. Always declare lessonMode. Use only the lesson steps and scene beats required for clarity, with at most ${DEFAULT_SYSTEM_LIMITS.maxLessonSteps} Learning Plan steps and ${DEFAULT_SYSTEM_LIMITS.maxSceneBeats} scene beats. Avoid filler or unnatural compression and group closely related concepts when needed. Use explain when the learner asks to understand, learn, see, or be shown a concept: provide complete example code and progressive explanation beats, not a workspace full of TODOs. When an initial scene teaches source code, preserve Monaco precision by giving each visual explanation beat one semantic target for the exact token, single line, or contiguous multi-line range being discussed, together with the registered highlight, spotlight, focus, or pointer effects that communicate that target. Treat endColumn as the exclusive endpoint after the final target character. Use practice only when the learner asks for an exercise, challenge, TODO, or hands-on work. Use mixed for a demonstration followed by a small validated exercise. Map every coding-exercise beat, including an intermediate exercise, to a lesson step with structural validation criteria and an editor-change interaction wait; Lessonique validates it in the same beat and gates Next or Finish. Use at most ${DEFAULT_SYSTEM_LIMITS.maxVisualGuideItems} final-exercise criteria and write each criterion's learner-visible requirement alongside its validator. Omit supportingItems from the final beat: Lessonique derives the numbered list from those criterion requirements in the same order, so the guide and validation progress cannot diverge. ${GUIDE_INLINE_CODE_SYNTAX_DESCRIPTION}`,
  },
  reset_classroom: {
  ...
  play_teaching_scene: {
    ...
    description: `Start a visual teaching scene using semantic targets and registered guidance capabilities. Use only the beats required for clarity, with at most ${DEFAULT_SYSTEM_LIMITS.maxSceneBeats}; avoid filler or unnatural compression and group closely related concepts when needed. Author one small concept per explanation beat, set lessonStepId to the Learning Plan section taught by each micro-step, and target the narrowest useful token or expression. Treat endColumn as the exclusive endpoint after the final target character. Set allowManualNavigation for learner-controlled Previous, Next, and Finish. Interaction beats are for learner work, must include a registered wait, and automatically remove invasive guidance. A coding exercise must map to criteria and use an editor-change wait so it exposes Validate Exercise and gates its own Next or Finish action; validation and feedback beats check and respond without exposing arbitrary code execution. ${GUIDE_INLINE_CODE_SYNTAX_DESCRIPTION}`,
  },
